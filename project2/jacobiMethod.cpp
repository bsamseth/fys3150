...
double off_diagonal_sum(double** A, int n){
    /*
      Return the root of the square sum of all
      off-diagonal elements in matrix A.
    */
    double sum = 0;
    for(int i = 0; i < n; i++){
	for(int j = 0; j < n; j++){
...
void find_indexes_of_max(double** A, int n, int* l, int* k){
    /*
      Find the indexes l and k of the element in A with the
...
double max_Aij(double** A, int n){
    /* Return the max off-diagonal element of A */
    double max_ = 0.0;
    for(int i = 0; i < n; i++){
	for(int j = 0; j < n; j++){
...
int jacobiMethod(double** A, double** R, int n, double* lambda, double eps){
    /*
      Implementation of the Jacobi Method for finding eigenvalues and
      eigenvectors of the matrix A. Matrix R and vector lambda will be
      filled with eigenvectors and eigenvalues, respectively.
      For details on the specifics of the method, see the report and/or
      material referenced in said report. 
    */
