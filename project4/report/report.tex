...
\end{equation}
der  $s_k=\pm 1$, $N$ er antall partikler.
...
derfor bruke et antall sykluser på å nå likevekt \textit{før} vi
begynner å samle inn data til forventningsverdiene. Spørsmålet blir da
hvor mange sykluser vi skal bruke.
...
For lave temperaturer ($T = 1$) ser vi at det ordnede systemet
...
datasamlingen først etter ca. $3\e{4}$ sykluser. Et alternativ bruke
...
termalisert burde vi ikke se store svingninger. Vi kan dermed se at
hvis verdiene ikke har endret seg mer en en gitt prosentandel på et
gitt antall sykluser så antar vi at systemet er termalisert og vi kan
begynne å samle data.
...
tillater at endres. Men selv om vi finner en ideell verdi for denne
kan det hende at vi bare ved en tilfeldighet ender med nye verdier som
er veldig like de vi hadde tidligere, selv om systemet ikke har nådd
den mest sannsynlige tilstanden. Dette er likevel ikke altfor
sannsynlig dersom vi er litt strenge, så vi ``krysser fingrene'' og
håper på det beste. 

Det kan nevnes at dette kan gjøres mye mer robust ved å bruke
autokorrelasjonsfunksjonen, men vi har ikke brukt dette i denne
rapporten av hensyn til tid.
...
tilstander modellen er i for forskjellige temperaturer, og hvor ofte disse
forekommer. I figur \ref{fig:probabilities} ser vi et histogram av tilstandene
for $n=20$ ved temperaturene $T=1$ og $T=2.4$. Vi ser at ved lav temperatur
skjer det relativt lite i systemet, og vi har få hopp. Dette er tilsvarende
det vi så i \ref{fig:configurations}. Ved høyere temperatur ser vi derimot
...
\begin{figure}[ht]
  ...
  \caption{Et histogram av tilstandene for $n=20$ ved temperaturene to 
...
\end{align} 
der $a$ er en konstant og $\nu$ definert i likning
(\ref{eq:corr-length-critical}). 
