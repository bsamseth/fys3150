...
oscillatorpotensialet.}
\label{tab:1}
\vspace{0.1cm}
\begin{tabular}{llll}
$\omega_r$ & $E(\omega_r)$ & $n_{step}$ & $\rho_{max}$\\  
...
Vi ønsker også å sammenligne de Schr\"odinger-ligningene med hverandre. Detta har vi valgt å gjøre for 
grunntilstanden og de to første eksiterte tilstandene. Resultatene kan sees i \ref{fig:three-lowest-states-non-interacting} og \ref{fig:three-lowest-states-interacting}.
...
å tro at hvis vi hadde utnyttet dette kunne kjøretiden vært forbedret.
Vårt valg av $\rho_{max}$ kommer også fra prøving og feiling. Kanskje dette kunne vært gjort på en lurere måte. 

Ellers ser resultatene fornuftige ut, og metoden funker godt til sitt bruk.
